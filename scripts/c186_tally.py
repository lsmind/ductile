#!/usr/bin/env python3
# c186_tally.py — 27B 问询链v2：三轮盲评聚合
import json
...
for r in (0, 1, 2):
    m = json.load(open(f"{TB}/c186_map_r{r}.json"))
    inv = {v: k for k, v in m.items()}
    raw = open(f"{TB}/c186r{r}_c181_s2.txt").read()
...
print()
print(f"g27c2 vs gbar: {sum(scores['g27c2'])/3 - sum(scores['gbar'])/3:+.1f}（v1 为 -7.3）")
print(f"g27c2 vs gc2:  {sum(scores['g27c2'])/3 - sum(scores['gc2'])/3:+.1f}")
print("C186-DONE")
