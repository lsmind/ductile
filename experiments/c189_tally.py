...
for r in (0, 1, 2):
    m = json.load(open(f"{TB}/c189_map_r{r}.json"))
    inv = {v: k for k, v in m.items()}
    raw = open(f"{TB}/c189r{r}_c181_s2.txt").read()
...
print()
print(f"g9d5 vs gbar: {sum(scores['g9d5'])/3 - sum(scores['gbar'])/3:+.1f}（g9c 42.7 / g9d4 64.0；目标≥0）")
print(f"g9d5 vs gc2:  {sum(scores['g9d5'])/3 - sum(scores['gc2'])/3:+.1f}")
print("C189-DONE")
